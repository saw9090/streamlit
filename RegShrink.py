...
def Generate_graph() :
    ...
    beta_1_min_basic = model.coef_[0]
    beta_2_min_basic = model.coef_[1]
    ...
    legend_text_basic_1 = f'OLS estimator (β1={beta_1_min_basic:.4f}, β2={beta_2_min_basic:.4f})'
    ...
    fig.add_trace(go.Scatter(x=boundary_x, y=boundary_y, mode='lines', name=f'{option} Boundary', line=dict(color='red'), showlegend=False)) # 페널티 경계선

    fig.update_layout(
        xaxis_title='β₁',
        yaxis_title='β₂',
        title_text=f'Loss Function Contour with<br>{option} Penalty Boundary',
        title_font=dict(size=24),
        width = 500,
        height = 500
    )
    ...
    fig.add_trace(go.Scatter(x=[0, 0], y=[np.min(beta_2_range), np.max(beta_2_range)], mode='lines', line=dict(color='black', width=1), showlegend=False))
    fig.add_trace(go.Scatter(x=[np.min(beta_1_range), np.max(beta_1_range)], y=[0, 0], mode='lines', line=dict(color='black', width=1), showlegend=False))
